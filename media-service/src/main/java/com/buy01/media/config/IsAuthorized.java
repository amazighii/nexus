...
public class IsAuthorized extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (!request.getRequestURI().startsWith("/api/media")) {
        ...
        if (userId == null || userRole == null) {
            writeError(response, HttpServletResponse.SC_UNAUTHORIZED, "Unauthorized",
                    "Authentication is required to access media resources.");
            return;
        }
    ...
    // test auto pipeline build via jenkins using github webhook 2
